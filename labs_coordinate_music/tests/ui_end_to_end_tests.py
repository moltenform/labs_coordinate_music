...
def testLinkSpotifyInteractive(tmpdir, mediadir):
    ...
    files.copy(
        mediadir + '/test_02_52.m4a', tmpdir + '/classic rock/collection/Queen - You\'re My Best Friend.m4a', False
    )
    parsedNames = [
        Bucket(
            short='Queen - You\'re My Best Friend.m4a',
            artist='Queen',
    ...
    files.copy(
        mediadir + '/test_03_31.m4a', tmpdir + '/classic rock/Queen/1975, A Night At The Opera/05 \'39.m4a', False
    )
    parsedNames.append(Bucket(short='05 \'39.m4a', artist='Queen', title='\'39', discnumber=1, tracknumber=5))

    tags = [
        CoordMusicAudioMetadata(tmpdir + '/classic rock/Queen/1975, A Night At The Opera/' + parsed.short)
    ...
    # add a url, which should be ignored
    writeUrlFile(
        tmpdir + '/classic rock/Queen/1975, A Night At The Opera/01 Ignored.url', 'spotify:track:7o4i6eiHjZqbASnSy3pKAq'
    )
    ...
    parsedNames.append(
        Bucket(
            short='01 06 You\'re My Best Friend.m4a',
            artist='Queen',
    ...
    files.copy(
        mediadir + '/test_03_31.m4a', tmpdir + '/classic rock/Queen/2002, The Platinum Collection/03 03 Barcelona.m4a',
        False
    )
    parsedNames.append(
        Bucket(short='03 03 Barcelona.m4a', artist='Queen', title='Barcelona', discnumber=3, tracknumber=3)
    )

    tags = [
        CoordMusicAudioMetadata(tmpdir + '/classic rock/Queen/2002, The Platinum Collection/' + parsed.short)
        for parsed in parsedNames
    ]
    for tag, parsed in zip(tags, parsedNames):
        tag.short = parsed.short
    ...
    # add a url, which should be ignored
    writeUrlFile(
        tmpdir + '/classic rock/Queen/2002, The Platinum Collection/01 01 Ignored.url',
        'spotify:track:7o4i6eiHjZqbASnSy3pKAq'
    )
...
def testMusicToUrlInteractive(tmpdir, mediadir):
    ...
    assertWarnEq(
        'p01p Carly Rae Jepsen - Run Away With Me.m4a 0.0Mb 134k 00:01',
        getStringTrackAndPopularity(
            tmpdir, CoordMusicAudioMetadata(tmpdirsl + 'Carly Rae Jepsen - Run Away With Me.m4a'), 1
        )
    )
    assertWarnEq(
        'p12p Qua - Ritmo Giallo.mp3 0.0Mb 136k 00:02',
        getStringTrackAndPopularity(tmpdir, CoordMusicAudioMetadata(tmpdirsl + 'Qua - Ritmo Giallo.mp3'), 12)
    )
    assertWarnEq(
        'p00p Masato Nakamura - Green Hill Zone.mp3 0.0Mb 136k 00:02',
        getStringTrackAndPopularity(
            tmpdir, CoordMusicAudioMetadata(tmpdirsl + 'Masato Nakamura - Green Hill Zone.mp3'), 0
        )
    )
...
def testFromOutsideMp3Interactive(tmpdir, mediadir):
    ...
    files.copy(
        mp3, tmpdirsl + 'outside/The Essential Fifth Dimension/01 17 Workin\' on a Groovy Thing__MARKAS__144.mp3', False
    )
    ...
    link1 = CoordMusicAudioMetadata(tmpdirsl + 'outside/Space Oddity/03 Letter To Hermione__MARKAS__128.mp3').getLink()
    link2 = CoordMusicAudioMetadata(
        tmpdirsl + 'outside/The Essential Fifth Dimension/01 17 Workin\' on a Groovy Thing__MARKAS__144.mp3'
    ).getLink()
    ...
    files.copy(
        wav, tmpdirsl + 'incoming/0001 Space Oddity' + '$David Bowie$03$Letter To Hermione$%s.wav' % link1, False
    )
    files.copy(
        wav, tmpdirsl + 'incoming/0002 The Essential Fifth Dimension' +
    ...
    coordmusictools.tools_newFilesBackToReplaceOutsideMp3s(
        tmpdirsl + 'outside',
        tmpdirsl + 'incoming',
    )
    ...
    assertWarnEq(
        [
            '/Space Oddity/03 Letter To Hermione.m4a', '/Space Oddity/05 Janine.url',
            '/Space Oddity/97 Nonexistent song 2.mp3', '/Space Oddity/98 Nonexistent song 3 (12).mp3',
    ...
    assertWarnEq(
        '97',
        CoordMusicAudioMetadata(tmpdirsl + 'outside/Space Oddity/97 Nonexistent song 2.mp3').get('tracknumber')
    )
    assertWarnEq(
        '98',
        CoordMusicAudioMetadata(tmpdirsl + 'outside/Space Oddity/98 Nonexistent song 3 (12).mp3').get('tracknumber')
    )
    assertWarnEq(
        '99',
        CoordMusicAudioMetadata(tmpdirsl + 'outside/Space Oddity/99 Nonexistent song 4.mp3').get('tracknumber')
    )
    assertWarnEq(
        'spotify:notfound',
    ...
    setOfLinks = set(
        [
            CoordMusicAudioMetadata(tmpdirsl + 'outside/Space Oddity/03 Letter To Hermione.m4a').getLink(),
            getFromUrlFile(tmpdirsl + 'outside/Space Oddity/05 Janine.url'),
            CoordMusicAudioMetadata(tmpdirsl + 'outside/Space Oddity/99 Nonexistent song 4.mp3').getLink(),
            CoordMusicAudioMetadata(
                tmpdirsl + 'outside/The Essential Fifth Dimension/01 17 Workin\' on a Groovy Thing.m4a'
            ).getLink(),
